    ...
    def _worker(target_symbols: list[str], access_token: str):
        ...
        for sym in target_symbols:
            ...
            if not inst:
                ...
                continue
            try:
                result = fetch_symbol(sym, inst["token"], access_token, force=False)
                if result == "cached":
                    _fetch_status["results"][sym] = "cached"
                elif result:
                    _fetch_status["results"][sym] = "ok"
                else:
                    _fetch_status["results"][sym] = "failed"
            except Exception as e:
                _fetch_status["results"][sym] = "failed"
...
def csv_status_route():
    """Check which symbols already have a CSV on disk — no token needed."""
    import glob
    ...
    for inst in INSTRUMENTS:
        ...
        for suffix in ["1YEAR_1MIN", "6MONTH_1MIN"]:
            p = os.path.join(data_dir, f"{sym}_{suffix}.csv")
            if os.path.exists(p) and os.path.getsize(p) > 1024:
                found = True
                break
        result[sym] = "ok" if found else "missing"
    have = sum(1 for v in result.values() if v == "ok")
    return jsonify({"results": result, "have": have, "total": len(result)})
...
def api_run_backtest():
    ...
    preset = {
        ...
        "swing_lookback":     int(  data.get("swing_lookback",     CONFIG.get("swing_lookback", 20))),
    }
